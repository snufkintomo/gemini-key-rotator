// Global fetch interceptor for unauthorized responses
        ...
        function renderTrends(data) {
            ...
            tokensChart = new Chart(ctxTok, {
                ...
                options: {
                    responsive: true,
                    plugins: {
                        title: { display: true, text: 'Token Consumption & Cache Savings (30 Days)' }
                    },
                    scales: {
                        y: { beginAtZero: true }
                    }
        ...
        function renderStatistics(stats) {
            ...
            const createKeyRow = (data, keyType) => {
                ...
                const enc = (val) => encodeURIComponent((val || '').toString());
                ...
                return `
                    <tr style="${rowStyle}">
                        ...
                        <td style="padding: 8px; border: 1px solid var(--border-color); text-align: center;">
                            <button class="btn-secondary-outline" onclick="diagnoseKeyFromStatsEncoded('${enc(rawUserToken)}', '${enc(data.raw_key)}', ${isOAuth}, ${isAntigravity}, '${enc(data.model)}', this)">Test Key</button>
                        </td>
                    </tr>
                `;
            };
        ...
        window.diagnoseKeyFromStats = async function(token, key, isOAuth, isAntigravity, model, button) {
        ...
        window.diagnoseKeyFromStatsEncoded = function(encToken, encKey, isOAuth, isAntigravity, encModel, button) {
            const token = decodeURIComponent(encToken || '');
            const key = decodeURIComponent(encKey || '');
            const model = decodeURIComponent(encModel || '');
            return window.diagnoseKeyFromStats(token, key, isOAuth, isAntigravity, model, button);
        };
